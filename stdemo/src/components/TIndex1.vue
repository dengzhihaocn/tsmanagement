<template>
        ...
        <el-scrollbar>
          <el-menu :default-openeds="['1', '3']" >
            <el-sub-menu v-for="(group, index) in meundata" :key="index" :index="group.groupname">
              <template #title>
                <el-icon><message /></el-icon>{{ group.groupname }}
              </template>
                <el-menu-item-group v-for="(items, itemsIndex) in group.items" :key="itemsIndex" :index="items.name"  >
                  <router-link :to="items.path"><el-menu-item :class="{ 'is-active': items.isactive}" @click="handleselect(items, itemsIndex)" >{{ items.name }}</el-menu-item></router-link>
                </el-menu-item-group>
            </el-sub-menu>
          </el-menu>
        </el-scrollbar>
  ...
  const handleselect = (items, itemsIndex:number) =>{
    items.isactive = !
    items.isactive;
    activeindex.value = itemsIndex;
  }
